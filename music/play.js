...
module.exports = async (msg, client, selecionada)=>{
	...
	storageService.getMusics().then(async musicas=>{
		if(voiceChannel){
			...
			let selectedMusic = musicas[(selecionada)] || (musicas[Math.floor(Math.random() * musicas.length)] || musicas[Math.floor(Math.random() * musicas.length)]);
			console.log(selectedMusic, selecionada);
